...
player_all_time = [c[1:] for c in player_all_time]
player_all_time = player_all_time + ordinary_players

# GET DATAFRAME FOR CURRENT DECK
# print(response.text.encode('utf8'))
# response = requests.get('https://api.clashroyale.com/v1/players/%23U200V9P', headers=headers)
# response2 = requests.get('https://api.clashroyale.com/v1/players/%23220U2GV', headers=headers)
...
# columns_keep = ['tag', 'expLevel', 'trophies', 'bestTrophies', 'wins', 'losses', 'battleCount', 'threeCrownWins', 'role',
# 'challengeCardsWon', 'challengeMaxWins', 'tournamentCardsWon', 'tournamentBattleCount']
# other_dat = pd.DataFrame()
# for i, player in enumerate(player_all_time):
#   url = 'https://api.clashroyale.com/v1/players/%23' + player
#   response = requests.get(url, headers=headers)
#   dic = json.loads(response.text)
#   new_dict = {key: dic[key] for key in dic.keys() if key in columns_keep}
#   if i == 0:
#     other_dat = pd.DataFrame(new_dict, index = [0])
#   else:
#     other_dat = other_dat.append(pd.DataFrame(new_dict, index = [i]))
# other_dat.dropna(inplace=True)
# print(other_dat)
# other_dat.to_csv('data.csv')
